...
for i in range(3):
    ...
    time.sleep(50)
    if exists(Pattern("1542893266942-1.png").similar(0.80),1) != None:
        break
find(Pattern("1542893266942-3.png").similar(0.80))
currentdate = datetime.datetime.now()
f = open(s, 'a')
f.write('Password for ' + c + ' accepted ' + currentdate.strftime("%H:%M") + '\n')
f.close()
click(Pattern("1542893266942-2.png").similar(0.80))
time.sleep(3)

waitVanish(Pattern("1543309633236-1.png").exact(),120)
currentdate = datetime.datetime.now()
f = open(s, 'a')
...
time.sleep(2)
if exists(Pattern("1543332827430-1.png").exact()) == None:
